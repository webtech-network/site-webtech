    ...
    return (
        <div className="flex justify-center flex-wrap gap-4">
            {events.map(event => {
                ...
                return (
                    <div key={event.id} className="flex flex-col justify-between gap-3 p-4 min-w-52 max-w-52 md:min-w-80 md:max-w-80 bg-cinza box-sombra-sm rounded-md md:bg-opacity-0">
                        <div className='flex flex-col justify-between'>
                            <img className="w-full h-40 hidden md:block" src={event.image} alt="" />
                            <h1 className="text-secundariaDark font-bold text-xl">{event.name}</h1>
                            <div className="">
