...
import { faServer, faLaptopCode, faRobot, faGlobe, faChartLine, faLightbulb } from '@fortawesome/free-solid-svg-icons';
        ...
        <section className="my-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {tracks.map((track, index) => (
                    <div 
                        key={index}
                        className="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 border border-gray-100 group"
                    >
                        <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-6 ${track.bgColor} group-hover:scale-110 transition-transform duration-300`}>
                            <FontAwesomeIcon icon={track.icon} className={`text-3xl ${track.color}`} />
                        </div>
